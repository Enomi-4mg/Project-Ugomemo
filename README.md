...
- 公開サイトは`docs/`配下の静的HTML/CSSで管理します。
- 英語ページは`docs/index.html`、日本語ページは`docs/ja/index.html`です。
- 共通スタイルは`docs/pages.css`に置きます。
- サイトは詳細仕様書ではなく、初見ユーザー向けの概要、現在の状態、試し方、サンプル、技術構成への導線を担当します。
- 最新の配布情報を書く場合は、GitHub Releasesの実在するタグとアセットを確認してから反映します。
- Samplesは、実際に`docs/assets/`へ存在する画像/動画/プロジェクトファイルだけを参照します。未確認の直接ダウンロードリンクや存在しない`.upj`は追加しません。
- 2026-06-16時点では、最新テストリリースは`v0.1.5-test`で、macOS Apple Silicon向け`.dmg`、Windows x64向け`.exe` / `.msi`がGitHub Releasesにあります。
- 2026-06-16時点では、掲載中のサンプルは`docs/assets/sample-animation.mp4`の1件です。

## ドキュメント

- [エージェント向け作業ガイド](./AGENTS.md)
- [設計書](./docs/DESIGN.md)
- [変更履歴](./CHANGELOG.md)
- このリポジトリでは、2026-06-16時点でルートの`AGENTS.md`をcoding agent向け作業ガイドとして使います。`skills.md`はありません。
