...
- Draw/Edit Mode中の`Cmd/Ctrl+C`: 現在フレームコピー
- Draw/Edit Mode中の`Cmd/Ctrl+V`: 現在フレームへペースト
- Edit Mode中の`Delete` / `Backspace`: 現在フレーム削除
- Audio Mode中の`Delete` / `Backspace`: 選択クリップ削除
- Audio Mode中の`Cmd/Ctrl+C`: 選択クリップコピー
- Audio Mode中の`Cmd/Ctrl+V`: 再生ヘッド位置へクリップペースト
- `T`: Tone
- `E`: Eraser
- `P`: Pen
- `S`: Shape
...
左列に描画ツール、中央に描画キャンバス、右列にレイヤー・背景・Z深さ・レイヤーClearを表示する。Cmd/Meta押下中だけ、前後ページの同じレイヤーを薄く表示する。
...
オニオンスキンは前後ページ全体ではなく、現在の `activeLayerId` と同じレイヤーだけを描画する。前ページはやや濃く、次ページはより薄く表示する。通常時は表示せず、Cmd/Meta押下中だけ有効にする。
