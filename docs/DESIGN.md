...
大きなReact UIは段階的に分割する。現時点では `src/ui/constants.ts` に安定したUI定数、`src/ui/types.ts` に共有UI型、`src/ui/components/IconButton.tsx` に小さなpresentational component、`src/ui/components/ProjectPreviewFrame.tsx` にプロジェクト解像度プレビューのcontain-style fit、`src/ui/keyboard/shortcuts.ts` にキーボード処理、`src/ui/tone/tonePattern.ts` にTone pattern helperを移した。Audio、Export、Timeline、Tauri副作用まわりのstate ownershipは安全性を優先して引き続き主に `App.tsx` に置く。
...
Draw ModeのメインキャンバスとAudio Modeのフレームプレビューは、常に `project.width / project.height` を表示縦横比のsource of truthにする。内部キャンバス解像度は変更せず、`ProjectPreviewFrame` が利用可能なUI領域を計測してobject-fit: contain相当の最大サイズを計算する。X/Yを別々に伸縮してはならず、余白は許容するがクロップはしない。ピクセルプレビューは `image-rendering: pixelated` を維持し、Draw Modeのポインタ座標は表示後のcanvas bounding rectからプロジェクト座標へ変換する。
...
- Save / Save As は Primary + S / Primary + Shift + S。macOSではCommand、Windows / LinuxではCtrlを使い、ブラウザやWebViewの標準保存動作は抑止する。
- Mode切り替えはプラットフォームPrimaryではなく物理Controlを使う。`Control+1` は Draw Mode、`Control+2` は Edit Mode、`Control+3` は Playback Mode、`Control+4` は Audio Mode。
- Draw ModeのTool切り替えは `Q` = Pen、`W` = Tone、`E` = Eraser、`R` = Shape。これらはDraw Modeに限定し、Edit / Playback / Audio ModeではTool変更しない。
- Brush sizeは `]` で縮小、`[` で拡大する。一般的なエディタと逆の割り当てだが、仕様としてこの対応を維持する。サイズは既存の最小/最大範囲に収める。
- ショートカットは `input` / `textarea` / `select` / `contenteditable` 内で入力中は無視する。
- `Up`: アクティブレイヤーを上へ移動
- `Down`: アクティブレイヤーを下へ移動
- `Left`: 前ページへ移動
- `Right`: 次ページへ移動
...
Saveは現在追跡している`.upj`パスへ直接上書きする。Save AsはTauri v2のネイティブSave Dialogから開始し、ユーザーのDocuments配下にプロジェクトパッケージを作る。Save / Save Asショートカットは `src/ui/keyboard/shortcuts.ts` のplatform-aware labelと検出ヘルパーを使う。LoadはTauri v2のネイティブOpen Dialogで`.upj`を選び、Rust側でzipを開いて`metadata.json`を読み、各レイヤーPNGをRGBAにデコードし、Audio Mode stateと一緒にReactへ返す。Dirty状態は上部バーの`Save *`とOSウィンドウタイトル末尾の`*`で表示する。Load / Save / Export中はブロッキングオーバーレイを表示し、アプリ全体への入力を遮断する。
...
SnapはAudio Mode編集時の配置補助であり、保存条件ではない。`timelineClips` と `audioTracks.clips` の `startFrame` / `durationFrames` / `sourceOffsetFrames` / fade frame値は小数フレームを保持できる数値としてRust側でも受け取り、off-snapの音声素材やクリップを強制スナップ、削除、丸め保存しない。NaNやInfinityのような非有限値だけは保存前に安全な数値へ正規化する。
...
アイコンボタンは共通の `IconButton` パターンで `aria-label` と native `title` を必ず持つ。hover時は既存のdeep pink反転に加えてアイコンを軽く持ち上げ、視覚的な反応とブラウザ標準ツールチップの両方で操作内容を確認できるようにする。Draw Tool、Mode、Save / Save Asなど主要アクションは `src/ui/keyboard/shortcuts.ts` の集中管理されたショートカットラベルを使い、`Pen (Q)`、`Draw mode (Ctrl+1)`、`Save (⌘S)` のように操作名とキーを一緒に表示する。
